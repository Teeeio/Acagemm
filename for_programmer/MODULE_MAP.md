...
| `scripts/` | 验证、E2E、运行报告和本地环境辅助工具 | `verify-*.mjs`, `e2e-*.mjs` | 源码、环境变量、Node/Python | 验证结果、测试报告 | PowerShell、npm、Python | Release Guard、E2E | 已实现 |
| `test-fixtures/` | `reference-fixture` 的隔离工作区代码模板 | `mla-kernels/` | 测试夹具重置 | 初始工作区文件 | 文件系统 | Smoke | 仅供测试，不是产品数据源 |
