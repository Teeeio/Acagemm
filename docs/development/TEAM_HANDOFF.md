...
> **最新入口（2026-09-16，标准 smoke / N20 收尾）**：生产修复源 `69747e1`（验收提交
> `6bf7a88`，329 个参与文件与 R8 完全相同）标准 affine smoke 通过；新 N20 原定
> 20 次全部执行，19 次完整成功 / 1 次预算终止，43/44 必需模型身份观测，严格验收失败。
> 第 15 次保留三个候选代码/正确性失败、无有效资料的研究和最后一次总预算取消；62 个
> N20 GPU 任务均已释放。原件独立复核、失败链和下一开发边界见单一事实源
> [`evidence/experience-study-20260915/status.json`](evidence/experience-study-20260915/status.json)
> 的 `standardSourceAcceptance`。不得用旧 N20 或 R8 补样本；不得称当前源稳定性已达标。
> 本轮由主代理直接执行，未使用 dispatch。下一步审查连续失败后的策略收敛，再冻结新源验收。
>
> **此前入口（2026-09-16，R8 三条件实机验收）**：修复源 `69747e1` 的九槽位试验
> 已完整通过并独立复核，每种经验条件 3 次；19/19 必需模型身份实际观测，28 个 GPU
> 任务均确认释放。保留一次正确性失败及原预算内恢复。原件、描述性比较及限制见
> [`evidence/experience-study-20260915/status.json`](evidence/experience-study-20260915/status.json)
> 的 `r8DirectAcceptance`。这是对照试验，不是新 N20；下一步独立标准 smoke / N20。
>
> **此前入口（2026-09-16，R7 策略拒绝修复）**：高风险候选仍被拒绝；自动编排现将
> 运行完成观测与需要人工介入的状态一起持久化，后续 tick / 重启不再反复应用同一候选。
> release 149 / non-hardware 44 通过。修复原件及边界见单一事实源
> [`evidence/experience-study-20260915/status.json`](evidence/experience-study-20260915/status.json)
> 的 `patchPolicyRejectionRepair`。下一步冻结新源、独立实机验收；旧 R7/N20 结论不扩展。
> 本轮由主代理直接实施，未使用 dispatch 平台。
>
> **此前入口（2026-09-16，直接实机 R7 收尾）**：Windows 进度写入修复已提交 `906832a`，
> 软件检查 release 149 / non-hardware 44 通过。该源 R7 因高风险候选被正确拒绝后，
> 编排未持久化阻塞状态、重复准备应用 102 次而预算终止；2/2 模型已实际观测。
> 原件独立复核、资源释放及下一修复边界见单一事实源
> [`evidence/experience-study-20260915/status.json`](evidence/experience-study-20260915/status.json)
> 的 `r7DirectAcceptance`。R7 保留 1 失败 / 8 未启动，无新 N20；未使用 dispatch 平台。
>
> **此前入口（2026-09-16，直接实机 R6 与 Windows 状态写入）**：修复源 `12a3c2a` 的
> R6 首次运行完成两个 GPU 候选，但 5 次必需模型观测中 1 次仅有思考流、预算取消前
> 无 assistant 模型元数据，故试验保持 1 失败 / 8 未启动。第二个候选还出现 Windows
> 原子进度写入 WinError 5；独立文件句柄已复现该错误，短时占用的有界重试与验收记录
> 见单一事实源 `evidence/experience-study-20260915/status.json`。本次由主代理直接执行，
> 未使用 dispatch 平台，旧 N20 不适用于本次源码。
>
> **此前入口（2026-09-16，经验收集冷查询修复）**：自动续轮与手动启动在经验收集前，
> 先刷新并核对 GPU 环境摘要。独立预检最多 30 秒，计入原 Mission/轮次总预算；
> 经验收集仍限 3 秒，原有证据校验继续执行。软件验收与只读实机诊断以
> [`evidence/experience-study-20260915/status.json`](evidence/experience-study-20260915/status.json)
> 的 `environmentPreflightRepair` 为准。本次由主代理直接实施，未派发 dispatch 下游；
> 尚无该修复版本的新实机 E2E/N20，下一步应冻结新源码、单独启动新验收计划。
>
> **此前入口（2026-09-16，三条件实机与报告修复）**：冻结源 `13f2014` 的 R5 实机计划
