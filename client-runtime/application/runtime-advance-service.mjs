...
import { pendingMissionResources } from '../cancellation-contract.mjs';
...
export const createRuntimeAdvanceService = ({ autopilot, advanceIteration, iteration, reconcileWorkflowState, detectGuard = detectLoopGuard, releaseResources }) => {
  ...
  const advance = async ({ state }) => {
    if (resourceReleaseBarrier(state)) return { state, changed: false, actions: { autopilot: 'none', iteration: 'resource_release_pending' } };
