...
Constructor requires `missionState.selectMission` in addition to state-access
ports; a missing method throws TypeError. The default research-agent shape comes
from `mission-state-shapes.mjs`, not the state-store facade.
