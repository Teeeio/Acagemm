...
export const normalizeBaselineSource = (input = {}, body = {}) => {
  ...
  return {
    ...
    kind: baselineKind,
    repository,
    canonicalRepository: String(source.canonicalRepository || repository).trim(),
    transportRepository: String(source.transportRepository || '').trim() || null,
    transportMode: String(source.transportMode || '').trim() || null,
    mirrorVerified: source.mirrorVerified === true,
    commit,
    tree: String(source.tree || source.sourceTree || '').trim() || null,
